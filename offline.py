...
print("Start the offline test")

#Test
cap_o = cap = pyshark.FileCapture("./Chrome_1p.pcapng")
...
print("Final result of Chrome")
print(predict_flow)
cap_o.close()

cap_o = cap = pyshark.FileCapture("./Youtube_2p.pcapng")
#bpf_filter='(wlan.addr == a4:50:46:3d:fe:59) && (wlan.addr == dc:53:7c:35:7d:65) and data
#differentiate between flows
print("YOUTUBE test")
features_packet=[]
act_pack_len_u=[]
...
for packet in cap_o:
    if len(packet.layers)==4 and packet.highest_layer=='DATA':
        if(packet.wlan.sa==MAC_MOBILE):
            ...
            inter_arrival_act_u= float(packet.frame_info.time_delta)
            #inter_arrival_act_u = float(packet.frame_info.time_relative) - inter_arrival_act_u
            act_pack_len_u.append(int(packet.frame_info.len))
            act_pack_rate_u.append(float(packet.wlan_radio.data_rate))
            act_pack_ia_u.append(inter_arrival_act_u)
            ...
            if(packets_u==AV_PACKETS_ONLINE):
                print("Uplink")
                #features_packet = [[numpy.mean(act_pack_len_u), numpy.mean(act_pack_rate_u), numpy.mean(act_pack_ia_u)]]
                features_packet = [[numpy.mean(act_pack_len_u), numpy.mean(act_pack_rate_u)]]
                print(features_packet)
                act_pre=clf_u.predict(features_packet)
                print(act_pre)
            ...
            if(packets_d==AV_PACKETS_ONLINE):
                ...
                print(act_pre)
                predict_flow.append(act_pre)
print("Final result of Youtube")
print(predict_flow)
cap_o.close()

#Test
cap_o = cap = pyshark.FileCapture("./Whatsup_p.pcapng")
